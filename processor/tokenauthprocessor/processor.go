...
func (c *tokenCache) set(token string, valid bool, appID, appName, tenantID string) {
	ttl := c.invalidTTL
	if valid {
		ttl = c.validTTL
	}
...
func (p *tokenAuthProcessor) ConsumeTraces(ctx context.Context, td ptrace.Traces) error {
	...
	for i := 0; i < resourceSpans.Len(); i++ {
		...
		if p.validateToken(ctx, token) {
			...
			// Enrich app_id/tenant_id into resource attributes for downstream data isolation
			p.enrichIdentity(token, newRS.Resource().Attributes())
		} else {
...
func (p *tokenAuthProcessor) ConsumeMetrics(ctx context.Context, md pmetric.Metrics) error {
	...
	for i := 0; i < resourceMetrics.Len(); i++ {
		...
		if p.validateToken(ctx, token) {
			...
			// Enrich app_id/tenant_id into resource attributes for downstream data isolation
			p.enrichIdentity(token, newRM.Resource().Attributes())
		} else {
...
func (p *tokenAuthProcessor) ConsumeLogs(ctx context.Context, ld plog.Logs) error {
	...
	for i := 0; i < resourceLogs.Len(); i++ {
		...
		if p.validateToken(ctx, token) {
			...
			// Enrich app_id/tenant_id into resource attributes for downstream data isolation
			p.enrichIdentity(token, newRL.Resource().Attributes())
		} else {
...
func (p *tokenAuthProcessor) validateToken(ctx context.Context, token string) bool {
	_, _, ok := p.validateTokenWithAppID(ctx, token)
	return ok
}

// validateTokenWithAppID validates the token and returns the associated AppID
// and TenantID.
func (p *tokenAuthProcessor) validateTokenWithAppID(ctx context.Context, token string) (appID, tenantID string, valid bool) {
	if token == "" {
		return "", "", false
	}
	...
	if p.cache != nil {
		if entry, ok := p.cache.get(token); ok {
			return entry.appID, entry.tenantID, entry.valid
		}
	}
	...
	if p.cache != nil {
		p.cache.set(token, result.Valid, result.AppID, result.AppName, result.TenantID)
	}
	...
	return result.AppID, result.TenantID, result.Valid
}

// enrichIdentity adds the app_id and tenant_id from token validation to
// resource attributes. Since validateToken() was already called successfully
// before this method, the identity is guaranteed to be in cache (if caching is
// enabled) or can be retrieved from a lightweight re-validation.
func (p *tokenAuthProcessor) enrichIdentity(token string, attrs pcommon.Map) {
	...
	if p.cache != nil {
		if entry, ok := p.cache.get(token); ok && entry.valid && entry.appID != "" {
			...
			if entry.tenantID != "" {
				attrs.PutStr("tenant_id", entry.tenantID)
			}
			return
		}
	}

	// Fallback: re-validate to get identity (cache miss, should be rare)
	appID, tenantID, ok := p.validateTokenWithAppID(context.Background(), token)
	if ok && appID != "" {
		attrs.PutStr("app_id", appID)
		if tenantID != "" {
			attrs.PutStr("tenant_id", tenantID)
		}
	}
}

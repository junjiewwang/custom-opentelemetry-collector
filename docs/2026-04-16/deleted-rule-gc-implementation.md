...
新增 **9 个测试**（含 8 个子测试），覆盖 GC 物理删除、expired 状态转换、EverApplySucceeded 标志、Memory/Redis 两种 store 的物理删除：

| # | 测试名称 | 文件 | 结果 |
|---|---------|------|------|
| 1 | `TestReconcileOfflineTargetExpiresToExpired` | `reconcile_test.go` | ✅ PASS |
| 2 | `TestReconcileExpiredTargetRecoversWhenOnline` | `reconcile_test.go` | ✅ PASS |
| 3 | `TestGCPhysicallyDeletesEligibleRule` | `gc_test.go`（新建） | ✅ PASS |
| 4 | `TestGCSkipsRuleWithinRetention` | `gc_test.go` | ✅ PASS |
| 5 | `TestGCSkipsRuleWithNonTerminalTarget` | `gc_test.go` | ✅ PASS |
| 6 | `TestEverApplySucceededSetOnFirstApply` | `reconcile_test.go` | ✅ PASS |
| 7 | `TestMemoryRuleStorePhysicalDelete` | `gc_test.go` | ✅ PASS |
| 8 | `TestAllTargetsTerminal`（8 个子测试） | `gc_test.go` | ✅ PASS |
| 9 | `TestRedisRuleStorePhysicalDelete` | `redis_store_integration_test.go` | ✅ PASS |

**实施发现**：
- `RedisRuleStore.PhysicalDeleteRule` 使用 pipeline 批量删除，重复删除返回 `nil`（与 `MemoryRuleStore` 返回 `ErrRuleNotFound` 行为不同）
- `TestReconcileExpiredTargetRecoversWhenOnline` 需要使用较长的 `HeartbeatTTL`（5s）避免 offline detection loop 竞态条件，并通过 `require.Eventually` + `Heartbeat` 确保 agent 在 reconcile 时在线
...
共修改 **2 个文件、6 处改动**，TypeScript 编译通过（`tsc --noEmit` 零错误）：

| # | 文件 | 改动点 | 说明 |
|---|------|--------|------|
| 1 | `instrumentation.ts` | `InstrumentationTargetState` 类型 | 新增 `'expired'` |
| 2 | `instrumentation.ts` | `InstrumentationRuleSummary` 接口 | 新增 `expired_targets: number` |
| 3 | `instrumentation.ts` | `InstrumentationOperationSummary` 接口 | 新增 `expired_targets: number` |
| 4 | `InstrumentationPage.tsx` | `targetStateClass()` | 新增 `'expired'` → amber 琥珀色映射 |
| 5 | `InstrumentationPage.tsx` | Summary Cards 第 4 张卡片 | `'Failed / Offline'` → `'Failed / Offline / Expired'`，值加入 `expired_targets` |
| 6 | `InstrumentationPage.tsx` | Last Operation 区域 | Offline 之后新增 Expired 字段展示 |

**向后兼容**：所有 `expired_targets` 读取均使用 `|| 0` 防护，旧版后端无此字段时不会 NaN。
...
|---------|------|---------|--------|--------|
| GC/expired 专项单测 | ✅ 已完成 | `reconcile_test.go` + `gc_test.go`（新建）+ `redis_store_integration_test.go` | 中 | P0 |
| 前端 ExpiredTargets 展示 | ✅ 已完成 | `instrumentation.ts` + `InstrumentationPage.tsx` | 低 | P1 |
| 配置模板注释更新 | ✅ 已完成 | `config/template/config.yaml` | 低 | P2 |
...
```mermaid
graph TD
    A[核心功能实施<br/>✅ 已完成 2026-04-16] --> B[遗留1: GC/Expired 专项单测<br/>✅ 已完成 2026-04-29]
    A --> C[遗留2: 前端 ExpiredTargets 展示<br/>✅ 已完成 2026-04-29]
    A --> D[遗留3: 配置模板注释更新<br/>✅ 已完成 2026-04-29]
    A --> E[遗留4: runtime_snapshot 适配<br/>✅ 不需要]
    B --> F[全量测试通过<br/>✅ 9 个新测试 + 0 回归]
    
    style A fill:#69db7c,color:#fff,stroke:#69db7c
    style B fill:#69db7c,color:#fff,stroke:#69db7c
    style C fill:#69db7c,color:#fff,stroke:#69db7c
    style D fill:#69db7c,color:#fff,stroke:#69db7c
    style E fill:#69db7c,color:#fff,stroke:#69db7c
    style F fill:#69db7c,color:#fff,stroke:#69db7c
```

---

*本文档将随后续实施进展持续更新。最近更新：2026-04-29 — 所有遗留问题已全部完成。*

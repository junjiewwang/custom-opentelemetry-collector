...
### Sprint 5 — Hybrid Provider 对齐（✅ 已完成 2026-06-30）

- [x] Hybrid `WriteSpans` + `WriteLogRecords` + `WriteMetricPoints` 路由
- [x] ES provider 补齐 `WriteLogRecords`/`WriteMetricPoints` 委托方法

### Sprint 6 — ES ILM Rollover（🔲 部署后生效）

- [ ] 新部署 → ES ILM rollover → 新数据使用新字段名 `name`/`kind`/`startTimeUnixNano`
- [ ] 旧索引自然淘汰（按 retention 配置）

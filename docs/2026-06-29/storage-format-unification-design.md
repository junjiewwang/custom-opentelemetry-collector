...
- [ ] 新建 `storedmodel/stored_log.go`（StoredLogRecord 类型）
- [ ] `stored_to_public.go` 加 `StoredLogRecordToPublic`
- [ ] 改 ES `log_writer.go` / `log_reader.go` / `admin.go`

### Sprint 3 — Metric 信号

- [ ] 新建 `storedmodel/stored_metric.go`
- [ ] 同 Log，改造 ES metric_writer / metric_reader / admin

### Sprint 4 — PG Provider 对齐

- [ ] PG `trace_writer.go` / `trace_reader.go` → StoredSpan
- [ ] PG `log_writer.go` / `log_reader.go`
- [ ] PG `metric_writer.go` / `metric_reader.go`
- [ ] 删 `pg_reader_adapter.go` 中的转换
